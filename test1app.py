...
def process_pipeline(
    ...
    # ====================================================================
    # Stage 2: YOLO wall/floor segmentation (包含單一類別自動水平切分)
    # ====================================================================
    ...
    if models and models.get("seg") is not None:
        ...
        if seg_results.masks is not None:
            ...
            orig_wall = np.zeros((H, W), dtype=np.uint8)
            orig_floor = np.zeros((H, W), dtype=np.uint8)
            wall_detected = False
            floor_detected = False

            for xy, cls_id in zip(seg_results.masks.xy, seg_results.boxes.cls):
                poly = np.array(xy, dtype=np.int32)
                class_name = cls_names[int(cls_id)].lower()
                
                if "wall" in class_name or "牆" in class_name or int(cls_id) == 1:
                    cv2.fillPoly(orig_wall, [poly], 1)
                    wall_detected = True
                elif "floor" in class_name or "地" in class_name or int(cls_id) == 0:
                    cv2.fillPoly(orig_floor, [poly], 1)
                    floor_detected = True

            # --- 自動水平切分邏輯 ---
            split_y = None
            if floor_detected and not wall_detected:
                y_coords = np.where(orig_floor == 1)[0]
                if len(y_coords) > 0:
                    split_y = int(np.min(y_coords))
                    yolo_desc += " (僅偵測到地板，已自動以地板頂部切分牆壁)"
                    
            elif wall_detected and not floor_detected:
                y_coords = np.where(orig_wall == 1)[0]
                if len(y_coords) > 0:
                    split_y = int(np.max(y_coords))
                    yolo_desc += " (僅偵測到牆壁，已自動以牆壁底部切分地板)"

            if split_y is not None:
                wall_mask[0:split_y, :] = 1
                floor_mask[split_y:H, :] = 1
                
                yolo_visual = aligned_rgb.copy()
                overlay = yolo_visual.copy()
                overlay[wall_mask == 1] = (0, 0, 255)   
                overlay[floor_mask == 1] = (255, 0, 0)  
                cv2.addWeighted(overlay, 0.4, yolo_visual, 0.6, 0, yolo_visual)
                
                cv2.line(yolo_visual, (0, split_y), (W, split_y), (0, 255, 255), 3)
                cv2.putText(yolo_visual, "Auto-Split Line", (10, max(25, split_y - 10)), 
                            cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 255), 2)
            else:
                wall_mask = orig_wall
                floor_mask = orig_floor
            # -------------------------------
    ...
    # ====================================================================
    # Stage 4: High-temperature detection (僅限地板區域 + 精確輪廓遮罩)
    # ====================================================================
    hsv = cv2.cvtColor(aligned_thermal, cv2.COLOR_BGR2HSV)
    h_channel, s_channel, v_channel = cv2.split(hsv)

    valid_floor_pixels = v_channel[floor_mask == 1]
    
    if len(valid_floor_pixels) > 0:
        thresh_v = np.percentile(valid_floor_pixels, HOT_PERCENT)
        cond_bright = (v_channel >= thresh_v) & (floor_mask == 1)
    else:
        cond_bright = np.zeros_like(v_channel, dtype=bool)

    mask = np.zeros_like(v_channel, dtype=np.uint8)
    mask[cond_bright] = 255 
    ...
    if final_contours:
        ...
        cv2.putText(hot_visual, "High Temp (Floor)", (x, max(y - 10, 20)), 
                    cv2.FONT_HERSHEY_SIMPLEX, 0.6, BOX_COLOR, 2)

    high_temp_area = np.count_nonzero(valid_hot_mask)
    high_temp_ratio = round((high_temp_area / (H * W)) * 100, 2) 

    # ====================================================================
    # Stage 5: low-temperature detection (僅限牆壁區域)
    # ====================================================================
    ...
    if COLD_MODULE_READY:
        try:
            ...
            _, cold_mask = detect_cold_regions(
                aligned_thermal,
                cold_model,
            ...
            # --- 確保冷區只發生在牆壁 ---
            cold_mask = cold_mask & (wall_mask == 1).astype(cold_mask.dtype)
            
            cold_visual = aligned_thermal.copy()
            cold_overlay = cold_visual.copy()
            cold_overlay[cold_mask > 0] = (255, 0, 0) # 藍色標示低溫
            cv2.addWeighted(cold_overlay, 0.5, cold_visual, 0.5, 0, cold_visual)

            cold_ratio = round((np.count_nonzero(cold_mask) / (H * W)) * 100, 2)
        except Exception as e:
